...
import { HandSign, IMatch, IMatchResolve, IMatchTransient, MatchResolveType, MatchResult, MatchStatus, PlayerMoves, IBaseMatch } from './shared/match.interface'
...
type IMatches = Record<string, IMatch>
...
export class MatchesService implements OnDestroy {
  ...
  private _updateMatches(fetchedMatches: IMatches, resolvedMatchAddresses?: string[]) {
    ...
    for (const match of Object.values(fetchedMatches)) {
      if (!equal(this._toBase(this.matches[match.address]), match)) {
  ...
  private _toBase(match: IMatch & IMatchTransient): IBaseMatch {
    const baseMatch = { ...match }
    delete baseMatch.isRevealing
    delete baseMatch.revealed
    delete baseMatch.owns
  ...
  private _resolveMatch = (match: IMatch, newMatch: IMatch): MatchResolveType => {
    ...
    if (match.status !== MatchStatus.Done && newMatch.status === MatchStatus.Done) {
      ...
      if (newMatch.result === MatchResult.Opponent) {
        if (match.opponent && match.opponent.address === this.user.address) {
